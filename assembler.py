...
rtype_re = re.compile(
    r'''^(?P<instr>(add|addu|sub|subu|and|or|xor|nor|slt|sltu))\s+(?P<rd>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|fp|sp|ra))\s+(?P<rs>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|fp|sp|ra))\s+(?P<rt>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|fp|sp|ra))$''')
shift_re = re.compile(
    r'''^(?P<instr>(sll|srl|sra))\s+(?P<rd>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|fp|sp|ra))\s+(?P<rt>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|fp|sp|ra))\s+(?P<immed>-?(0x)?[0-9a-fA-F]+)$''')
immed_re = re.compile(
    r'''^(?P<instr>(addi|addiu|slti|sltiu|andi|ori|xori))\s+(?P<rt>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|fp|sp|ra))\s+(?P<rs>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|sp|fp|ra))\s+(?P<immed>-?(0x)?[0-9a-fA-F]+)$''')
lui_re = re.compile(
    r'''^(?P<instr>lui)\s+(?P<rt>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|fp|sp|ra))\s+(?P<immed>-?(0x)?[0-9a-fA-F]+)$''')
mem_re = re.compile(
    r'''^(?P<instr>(lw|sw))\s+(?P<rt>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|fp|sp|ra))\s+(?P<immed>-?(0x)?[0-9a-fA-F]+)\s*\(\s*(?P<rs>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|fp|sp|ra))\s*\)$''')
j_re = re.compile(r'''^(?P<instr>(j|jal))\s+(?P<label>\w+)$''')
branch_re = re.compile(
    r'''^(?P<instr>(beq|bne))\s+(?P<rs>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|sp|fp|ra))\s+(?P<rt>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|sp|fp|ra))\s+(?P<label>\w+)$''')  # note switch
jr_re = re.compile(
    r'''^(?P<instr>(jr))\s+(?P<rs>\$(0|zero|at|v[0,1]|a[0-3]|t[0-9]|s[0-7]|k[0-1]|gp|fp|sp|ra))$''')
signed_re = re.compile(r'addi|addiu|slti|sltiu|beq|bne|lw|sw')
both_allowed_re = re.compile(r'andi|ori|xori')
    ...
    for line in inputFile:
        ...
        if len(instruction) != 0:
            ...
            if rtype:
                ...
                debug("instruction: %s rtype: rs: %d rt: %d rd: %d funct:%d num: %04x" % (
                    instruction, rs, rt, rd, funct, num))
            elif shift:
                ...
                num = 0 << 26 | 0 << 21 | rt << 16 | rd << 11 | (
                    immediate & 31) << 6 | funct
                debug("instruction: %s rtype: rs: %d rd: %d shamt: %d funct:%d num: %04x" % (
                    instruction, rs, rd, immediate, funct, num))
            elif immed:
                ...
                debug("instruction: %s rs: %d rt: %d opcode: %d immediate: %d num: %04x" % (
                    instruction, rs, rt, opcode, immediate, num))
            elif lui:
                ...
                debug("instruction: %s rt: %d opcode: %d immediate: %d num: %04x" % (
                    instruction, rt, opcode, immediate, num))
            elif mem:
                ...
                if label not in symbols:
                    raise AssemblerSyntaxError(
                        lineNo, "unknown label %s" % j.group('label'))
                instructionNo = symbols[label]
                thisInstruction = instructionsSeen
                if thisInstruction & 0x3000000 != instructionNo & 0x3000000:
                    raise AssemblerRangeError(lineNo, "label %s is in another jump zone (instr: %04x, target:%04x)" % (
                        label, thisInstruction, instructionNo))
                # instructionNo += 0x100000
                num = opcode << 26 | (instructionNo & 0x3FFFFFF)
                debug("instruction: %s addr: %d num: %04x" %
                      (instruction, instructionNo & 0x3FFFFFF, num))
            elif branch:
                ...
                if label not in symbols:
                    raise AssemblerSyntaxError(
                        lineNo, "unknown label %s" % branch.group('label'))
                instructionNo = symbols[label]
                offset = instructionNo - (instructionsSeen + 1)
                if offset > 2 ** 16 - 1 or offset < -(2 ** 16):
                    raise AssemblerRangeError(
                        lineNo, "label %s is too far away: %d instructions from pc+1" % (label, offset))
                num = opcode << 26 | rs << 21 | rt << 16 | (offset & 0xFFFF)
                debug("instruction: %s rs: %d rt: %d opcode: %d offset: %d num: %04x" % (
                    instruction, rs, rt, opcode, offset, num))
            elif jr:
                rs = registers[jr.group('rs')[1:]]
                funct = functs[jr.group('instr')]
                num = 0 << 26 | rs << 21 | 0 << 16 | 0 << 11 | 0 << 6 | funct
                debug("instruction: %s rs: %d num: %04x" %
                      (instruction, rs, num))
            else:
                raise AssemblerSyntaxError(
...
def print_instructions(instructions, depth, width, infile, outfile):
    print >> outfile, "DEPTH = %d;" % depth
    print >> outfile, "WIDTH = %d;" % width
    print >> outfile, "CONTENT"
    print >> outfile, "BEGIN"
    for i, line in enumerate(infile):
        print >> outfile, "%2x : %08x; %% (%02x) %s %%" % (i, instructions[i], i * 4, line[:-1])
    print >> outfile, "END ;"
...
if __name__ == "__main__":
    ...
    parser.add_option('-o', '--out', dest='output_file', type='string',
                      default='inst_mem.mif', help="Specify output filename")
    parser.add_option('-v', '--verbose', dest='verbose',
                      action='store_true', default=False, help='verbose debug mode')
    parser.add_option('-d', '--depth', dest='depth', type='int',
                      default=64, help='Specify memory depth')
    ...
    input_file = args[0]
    if re.match(r""".*(?P<extension>\.s)$""", input_file, re.I) and output_file == 'inst_mem.mif':
        output_file = input_file[:-1] + "mif"
    ...
    try:
        ...
        print_instructions(instructions, depth, width, infile, outfile)
        infile.close()
        outfile.close()
    except IOError, e:
